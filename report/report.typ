...
== Diffuse scattering
The Laue process is a common crystallographic method by which the Laue group of crystal and crystal parameters can be determined. The best use case for the Laue 
...
+ Compare the hamiltonians of the states.
  ...
  + If the hamiltonian is higher the step is accepted with probability $rho = exp(- (Delta cal(H))/(k_b T))$.
+ back to step 2)
...
== Diffraction
On the `.mmcif` files produced by the command line tool `gemmi sfcalc` is run. This program calculates the structure factors for the input file. It is integrated into a python script which then converts the data into the `Yell` format.
...
#align(center)[
  #figure(
    grid(columns: 3, row-gutter: 2mm, column-gutter: 1mm,
      image("../figs/examples/a.png"), image("../figs/examples/b.png"), image("../figs/examples/c.png"),
      "a)", "b)", "c)",
      image("../figs/examples/d.png"), image("../figs/examples/e.png"), image("../figs/examples/f.png"),
      "d)", "e)", "f)"
    ),
